...
works out of the box — 11 routes across **Polygon PoS, Arbitrum, OP Mainnet, Base,
and Scroll** (USDC / USDbC / DAI / WBTC / LINK / cbETH), each checked to be `BACKED`
(locked ≥ minted) before inclusion, with its escrow linked in `notes`. Add your own
(or point `EVMSEC_BRIDGES` at a private file); `npm run validate:registry` enforces
shape, checksummed addresses, and a cited source. ⚠️ Escrows and mappings **change** —
re-verify against each bridge's own contracts before relying on a number. A security
tool fed the wrong escrow lies confidently, so anything that didn't verify cleanly was
left out rather than shipped as a false alarm: USDT-on-Polygon (standard predicate
isn't the real escrow, reads ~1%), Arbitrum USDC.e (custom gateway, reads 0% against
the standard one), and Linea USDC (canonical bridge deprecated for native USDC) were
all tested and excluded. Linea and Scroll are also configured for ad-hoc use even
where a lock-and-mint route isn't registered.
...
The same run also emits a machine-readable feed ([`STATUS.json`](STATUS.json)), a
[shields.io](https://shields.io) endpoint ([`badge.json`](badge.json)) — that's the live
**bridges** badge at the top of this page — and a trend log
([`STATUS.history.jsonl`](STATUS.history.jsonl), one rollup per run). It commits the
artifacts back only when the numbers change.

**It pages you.** When a route actually goes undercollateralized, the workflow
**opens a GitHub issue** (labelled `bridge-breach`, one at a time) and **closes it on
recovery** — an unattended bridge-watch, no external monitoring service. Only a real
breach opens an issue; an unreadable RPC never cries wolf. That's the standalone
bridge-watch the ecosystem doesn't have as open source.
